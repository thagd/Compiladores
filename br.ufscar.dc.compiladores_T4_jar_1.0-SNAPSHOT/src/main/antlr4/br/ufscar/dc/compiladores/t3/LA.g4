...
identificador :	ident1=IDENT ('.' subIdent+=IDENT)* dimensao;
...
constantes : limiteEsq=numero_intervalo (',' limiteDir+=numero_intervalo)*;
...
parcela_unario : '^'? identificador | ident=IDENT '(' expressao (',' expressao)* ')' | NUM_INT | NUM_REAL | '(' exp=expressao ')';
